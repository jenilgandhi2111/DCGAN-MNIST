...
class Generator:
    ...
    def getGeneratorOptimizer(self,LR=3e-4):
...
class Discriminator:
    def getDiscriminator(self,INPUT_SHAPE=(28,28,1),BASE_FILTERS=64,KERNEL_SIZE=(5,5)):
        ...
        # Input Layer
        self.discriminator.add(Conv2D(BASE_FILTERS,kernel_size=KERNEL_SIZE,use_bias=False,strides=(2,2),padding="same"))
        self.discriminator.add(LeakyReLU())
        self.discriminator.add(Dropout(0.3))

        #Layer 1
        self.discriminator.add(Conv2D(BASE_FILTERS*2,kernel_size=KERNEL_SIZE,strides=(2,2),padding="same",use_bias=False))
        self.discriminator.add(LeakyReLU())
